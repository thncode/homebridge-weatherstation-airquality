...
var PPM25, PPM10, charge, battery, probes, readtime;
...
WeatherStationAirquality.prototype.readData = function () {
	...
	charge = parseFloat(data.substring(37)); 
	probes = parseFloat(data.substring(42));
	battery = parseFloat(data.substring(32));
	
	var ppm;
	if (PPM10 < 21) ppm = PPM10 * 15 + 400;
	...
	this.log("Air quality data: ", PPM25, PPM10, ppm, probes, charge, battery);

	this.fakeGatoHistoryService.addEntry({ time: moment().unix(), temp: 0, humidity: 0, ppm: ppm });

	this.airQualityService.getCharacteristic(Characteristic.PM2_5Density).updateValue(PPM25.toFixed(), null);
	this.airQualityService.getCharacteristic(Characteristic.PM10Density).updateValue(PPM10.toFixed(), null);
	this.airQualityService.getCharacteristic(Characteristic.AirQuality).updateValue(2, null);
    this.batteryService.getCharacteristic(Characteristic.ChargingState).updateValue(charging(), null);
...
WeatherStationAirquality.prototype.getChargingState = function (callback) {
    return callback(null, charging());
};
...
WeatherStationAirquality.prototype.getPM2_5Density = function (callback) {
    return callback(null, PPM25.toFixed());
};


WeatherStationAirquality.prototype.getPM10Density = function (callback) {	
    return callback(null, PPM10.toFixed());
};


WeatherStationAirquality.prototype.getAirQuality = function (callback) {	
												//	Characteristic.AirQuality.UNKNOWN = 0;
	if (PPM10 < 21) return callback(null, 1);   //	Characteristic.AirQuality.EXCELLENT = 1;
	if (PPM10 < 36) return callback(null, 2);   //	Characteristic.AirQuality.GOOD = 2;
	if (PPM10 < 51) return callback(null, 3);   //	Characteristic.AirQuality.FAIR = 3;
	if (PPM10 < 101) return callback(null, 4);  //	Characteristic.AirQuality.INFERIOR = 4;
	return callback(null, 5);                   //	Characteristic.AirQuality.POOR = 5;
};
